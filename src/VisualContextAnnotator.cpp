...
VisualContextAnnotator::detectObjectsWithCanny(const Mat &frame_gray,
                                               const double &lowThreshold,
                                               const Size &minSize, const cv::Size &maxSize) noexcept {
  ...
  for (auto &cnt : localContours) {
    ...
    if (approx.size() > 0) {
      Rect r = boundingRect(approx);
      if (r.size().width >= minSize.width && r.size().height >= minSize.height && r.size().width <= maxSize.width && r.size().height <= maxSize.height) {
        result.push_back(r);
      }
    }
  }
...
vector<Annotation> VisualContextAnnotator::predictWithTESSERACT(const Mat &frame_gray) noexcept {
  ...
  vector<Rect> detects = detectObjectsWithCanny(frame_gray, 77, Size(10, 10), Size(50, 50));
  if (detects.size() <= 0) {
  ...
  vector<Annotation> result;
  for (auto &&rect : detects) {
    auto annot =  predictWithTESSERACTInRectangle(rect, frame_gray);
    if(&annot != &EMPTY){
